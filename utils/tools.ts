    ...
    if (con) {
        if (con.includes('/:')) {
            for (var i = 0; i < emojis.length; i++) {
                con = con.replace(emojis[i].reg, '<img src="http://servers.365coding.cn/resource/image/emojis/' + emojis[i].file + '"  alt="" style="vertical-align: middle; width: 24px; height: 24px" />');
            } return con;
        }
    }
    return con;
